...
export function createCrmService(repository: CrmRepository) {
  ...
  async function upsertLead(input: UpsertLeadInput): Promise<Lead> {
    ...
    const record: Lead = {
      ...
      olegPercent: input.olegPercent ?? existing?.olegPercent ?? (existing ? null : 2),
      olegCommissionEnabled: input.olegCommissionEnabled ?? existing?.olegCommissionEnabled ?? !existing,
